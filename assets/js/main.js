...
require(['jquery', 'appSession', 'urlParser', 'select2'], function ($, appSession, urlParser, select2) {
	...
	******************************************************* */
	...
	if (urlParser.search.string === '') {
		...
		for (var i = 0; i < urlParser.params.length; i++) {
			var param = urlParser.params[i].split('=');
	...
	******************************************************* */
	...
	$('#update').click(function () {
		...
		require(['marked', 'less', 'htmlBeautify', 'jsZip'], function (marked, less, htmlBeautify, jsZip) {
			...
			var logger = '<script>var console={};window.onerror=function(msg,url,line){parent.document.getElementById("console-editor").insertAdjacentHTML("beforeend","<code class=\'js-error\'>> "+msg+" </code><br>")};';
